  ...
  namespace proc {
    ...
    void DiffVM::prepareKinematics() {
      ...
      if (igammd_ >= PhotonMode::WWA) {
        epa_calc_.init(pA(), pB(), kinematics().cuts().initial.q2, w_limits);
      ...
      if (vm_.lambda <= 0.)
        vm_.lambda = event().oneWithRole(Particle::CentralSystem).momentum().mass();

      const double q2_min = kinematics().cuts().initial.q2.min();
      prop_mx_ = std::max(1.,
                          vm_.xi * q2_min / (pow(vm_.lambda, 2) + vm_.xi * vm_.chi * q2_min) /
                              pow(1. + q2_min / pow(vm_.lambda, 2), vm_.eprop));

      const Particle& vm = event()(Particle::CentralSystem)[0];
    ...
    double DiffVM::computeWeight() {
      ...
      if (!kinematics().cuts().initial.q2.contains(q2))
        return 0.;
    ...
    void DiffVM::fillKinematics() {
      ...
      event().oneWithRole(Particle::CentralSystem).setMomentum(Momentum(p_vm_cm_).lorentzBoost(-p_cm_));
    }
    ...
    double DiffVM::computeT(double x, double b) const {
      const auto& t_range = kinematics().cuts().initial.q2;
      double bloc = b;
