...
namespace cepgen {
  namespace pythia8 {
    ...
    void EventInterface::feedEvent(const Event& ev) {
      const double scale = ev(Particle::Intermediate)[0].momentum().mass();
      ...
      const double x1 = q2_1 / (q2_1 + op1.momentum().mass2() - mp2_),
                   x2 = q2_2 / (q2_2 + op2.momentum().mass2() - mp2_);
      ...
      for (const auto& py_cg : pyid_vs_cgid) {  // 2nd loop to establish parentage
        ...
        if (cgpart.status() == Particle::Status::DebugResonance) {  // fix whatever status we can fix
          if (pypart.isResonance()) {
            if (cgpart.role() == Particle::CentralSystem && pypart.status() < 0)
