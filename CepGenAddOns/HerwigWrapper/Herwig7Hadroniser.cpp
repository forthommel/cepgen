...
namespace CepGen {
  namespace Hadroniser {
    ...
    HerwigHadroniser::HerwigHadroniser(const ParametersList& plist)
        : Hadroniser(plist),
          thepeg_(nullptr),
          run_mode_(Herwig::RunMode::READ),
          parallel_jobs_(plist.get<int>("numParallelJobs")),
          exit_on_error_(plist.get<bool>("exitOnError", true)),
          repo_location_(plist.get<std::string>("herwigPath")),
          run_(plist.get<std::string>("run")),
          generator_(plist.get<std::string>("generator")),
          repository_(fullPath(repo_location_, plist.get<std::string>("repository"))),
      ...
      CG_INFO("HerwigHadroniser") << "Initialising the Herwig core.\n"
                                  ...
                                  << "Repository: " << plist.get<std::string>("repository");
    }
    ...
    void HerwigHadroniser::init() {
      ...
      try {
        ...
        kCepGenParameters.reset(new Parameters(*params_));
    ...
    void HerwigHadroniser::setCrossSection(double xsec, double xsec_err) {}

    bool HerwigHadroniser::run(Event& ev, double& weight, bool full) {
      weight = 1.;
      ...
      try {
        ...
        for (const auto& ip : proc->intermediates())
...
REGISTER_MODIFIER("herwig7", HerwigHadroniser)
