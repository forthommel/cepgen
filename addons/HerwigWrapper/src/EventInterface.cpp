...
namespace cepgen::herwig {
  extern std::shared_ptr<Event> kCepGenEvent;
  extern RunParameters* kCepGenParameters;
}  // namespace cepgen::herwig
...
namespace ThePEG {
  ...
  void CepGenInterface::open() {
    if (!cepgen::herwig::kCepGenParameters) {
      CG_ERROR("CepGenInterface") << "Parameters block is not set.";
      throw Stop();
    }
    //--- beam particles information
    const auto& kinematics = cepgen::herwig::kCepGenParameters->kinematics();
    const auto& pos_beam = kinematics.incomingBeams().positive();
    const auto& neg_beam = kinematics.incomingBeams().negative();
    heprup.IDBMUP = std::pair<long, long>{pos_beam.integerPdgId(), neg_beam.integerPdgId()};
  ...
  bool CepGenInterface::doReadEvent() {
    ...
    auto evt = cepgen::herwig::kCepGenEvent;
    if (!evt)
      throw Stop();
    ...
    hepeup.IDPRUP = 0;
    hepeup.XWGTUP = 1.;
    hepeup.SCALUP = (*evt)(cepgen::Particle::Role::Intermediate)[0].momentum().mass();
    ...
    const auto& part1 = (*evt)[cepgen::Particle::Role::Parton1];
    const auto& part2 = (*evt)[cepgen::Particle::Role::Parton2];
  ...
  struct BaseClassTrait<CepGenInterface, 1> : public ClassTraitsType {
    typedef LesHouchesReader NthBase;  ///< Alias for the first base class
  };
  ...
  struct ClassTraits<CepGenInterface> : public ClassTraitsBase<CepGenInterface> {
    ...
    static inline std::string className() { return "ThePEG::CepGenInterface"; }
    /// Dynamic library location where the class CepGenInterface is implemented
    static inline std::string library() { return "libCepGenHerwig.so"; }
  };
}  // namespace ThePEG
