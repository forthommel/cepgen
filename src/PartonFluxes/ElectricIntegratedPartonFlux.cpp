...
namespace cepgen {
  class ElectricIntegratedPartonFlux : public IntegratedPartonFlux {
  public:
    explicit ElectricIntegratedPartonFlux(const ParametersList& params) : IntegratedPartonFlux(params) {}

    double mass2() const override { return mp2_; }
    bool fragmenting() const override final { return false; }
    spdgid_t partonPdgId() const override final { return PDG::photon; }
    ...
    double flux(double x) const override {
      ...
      return alpha_over_pi_ * (1. - x + 0.5 * x * x) / x *
             ((a + 3.) / (a - 1.) * std::log(a) - 17. / 6 - 4. * inv_a / 3. + inv_a * inv_a / 6.);
    }
  };
}  // namespace cepgen
