...
namespace cepgen {
  ...
  public:
    ...
    REGISTER_TYPE(std::vector<ParametersList>, map_vec_params_)
  ...
  public:
    ...
    inline const ParametersList& parameters() const override {
      ...
      for (const auto& kv : map_params_)
        params_.set(kv.first, kv.second.get());
      for (const auto& kv : map_vec_params_)
        params_.set(kv.first, kv.second.get());
      return params_;
    }
    virtual void setParameters(const ParametersList& params) override {
      ...
      for (const auto& kv : map_vec_params_)
        kv.second.get() = params_.operator[]<std::vector<ParametersList> >(kv.first);
    }
  };
}  // namespace cepgen
