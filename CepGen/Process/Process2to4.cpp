...
namespace cepgen {
  namespace proc {
    Process2to4::Process2to4(const ParametersList& params, std::array<pdgid_t, 2> partons, pdgid_t cs_id)
        : KTProcess(params, partons, {cs_id, cs_id}), single_limits_(params) {}
    ...
    double Process2to4::computeKTFactorisedMatrixElement() {
      ...
      return amat2 * pow(4. * x1_ * x2_ * s_ * M_PI, -2) * 0.25 * constants::GEVM2_TO_PB * (pc(0).pt() - pc(1).pt()) *
             qt1_ * qt2_;
    }

    void Process2to4::fillCentralParticlesKinematics() {
      static auto gen = std::bind(std::uniform_int_distribution<>(0, 1), std::default_random_engine());
      short sign = gen() ? +1 : -1;  // randomise the charge of outgoing system
